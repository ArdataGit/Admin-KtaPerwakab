...
@section('page-title', 'Edit Campaign Donasi')
...
@section('content')
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h2 class="text-2xl font-bold text-gray-800">Edit Campaign Donasi</h2>
            <p class="text-sm text-gray-600 mt-1">Perbarui informasi campaign donasi</p>
        </div>
    ...
    <div class="bg-white rounded-xl shadow-md overflow-hidden">
        <div class="bg-gradient-to-r from-yellow-500 to-yellow-600 p-6">
            <h3 class="text-xl font-semibold text-white">
                <i class="fas fa-edit mr-2"></i>Form Edit Campaign Donasi
            </h3>
        </div>

        <form action="{{ route('master.donation-campaign.update', $data->id) }}" method="POST" enctype="multipart/form-data" class="p-6">
            @csrf
            @method('PUT')

            @include('pages.master.donation-campaign.form', ['data' => $data])
